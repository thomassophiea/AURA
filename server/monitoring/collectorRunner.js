...
export async function collectSource({ source, config, now = new Date(), deps = {} }) {
  ...
  // The measured-power series is NOT collected here. It runs in its own loop
  // (energyApStateRunner.js) because this tick is skipped while the previous
  // one is still running, and the per-AP report collector routinely outlasts
  // the poll interval — which turned a 60s setting into 180-240s of real
  // sampling. See that module's header.
