...
// Strip trailing /api, /management, or /api/management — the proxy already adds
// the /management prefix when forwarding, so a suffixed env var produces
// double-prefixed URLs that 404 (e.g. https://host/management/management/v1/sites).
function normalizeControllerUrl(raw) {
  if (!raw) return raw;
  let url = raw.replace(/\/+$/, '');
  const stripped = url.replace(/\/(api\/management|management|api)$/i, '');
  if (stripped !== url) {
    console.warn(
      `[Proxy Server] ⚠  CAMPUS_CONTROLLER_URL had trailing path suffix; normalized "${raw}" → "${stripped}"`
    );
  }
  return stripped;
}
const DEFAULT_CONTROLLER_URL = normalizeControllerUrl(process.env.CAMPUS_CONTROLLER_URL);
...
if (!process.env.CAMPUS_CONTROLLER_URL) {
  if (process.env.NODE_ENV === 'production') {
    ...
    console.warn(
      '[Proxy Server] ⚠  CAMPUS_CONTROLLER_URL not set (dev mode - API proxy disabled, static assets only)'
    );
  }
} else {
...
if (!process.env.ALLOWED_ORIGINS) {
  if (process.env.NODE_ENV === 'production') {
    console.warn(
      '[Proxy Server] ⚠  ALLOWED_ORIGINS not set in production — CORS will allow all origins. Set ALLOWED_ORIGINS for tighter security.'
    );
  } else {
    console.warn(
      '[Proxy Server] ⚠  ALLOWED_ORIGINS not set — CORS will allow all origins (DEV ONLY)'
    );
  }
} else {
...
if (process.env.NODE_ENV === 'production') {
  if (process.env.NODE_TLS_REJECT_UNAUTHORIZED === '0') {
    console.warn(
      '[Proxy Server] ⚠  NODE_TLS_REJECT_UNAUTHORIZED=0 in production — this disables certificate verification'
    );
  }
    ...
    contentSecurityPolicy: {
      directives: {
        ...
        connectSrc: [
          ...
          'https://api-eu.extremecloudiq.com',
          'wss://*.supabase.co',
        ],
      },
    },
...
function isValidHost(host) {
  ...
  return (
    /^[a-zA-Z0-9][a-zA-Z0-9.\-]*[a-zA-Z0-9]$/.test(host) || /^(\d{1,3}\.){3}\d{1,3}$/.test(host)
  );
}
...
function tcpPing(host, port, timeout = 5000) {
  return new Promise((resolve) => {
    ...
    socket.on('timeout', () => {
      socket.destroy();
      resolve({ success: false, time: 0, port });
    });
    socket.on('error', () => {
      socket.destroy();
      resolve({ success: false, time: 0, port });
...
async function performPing(host, count) {
  ...
  try {
    ...
    const rttMatch = stdout.match(
      /(?:rtt|round-trip)\s+min\/avg\/max(?:\/mdev)?\s*=\s*([\d.]+)\/([\d.]+)\/([\d.]+)/i
    );
    return {
      host,
      ip,
      ...
      rtt: {
        min: rttMatch
          ...
          : results.length
            ...
            : 0,
        avg: rttMatch
          ? parseFloat(rttMatch[2])
          : results.length
            ? results.reduce((s, r) => s + r.time, 0) / results.length
            : 0,
        max: rttMatch
          ? parseFloat(rttMatch[3])
          : results.length
...
app.post(
  '/api/management/platformmanager/v1/network/ping',
  requireAuth,
  diagnosticRateLimit,
  jsonParser,
  async (req, res) => {
...
app.post(
  '/api/management/platformmanager/v1/network/traceroute',
  requireAuth,
  diagnosticRateLimit,
  jsonParser,
  async (req, res) => {
...
app.post(
  '/api/management/platformmanager/v1/network/dns',
  requireAuth,
  diagnosticRateLimit,
  jsonParser,
  async (req, res) => {
    ...
    try {
      ...
      try {
        ...
        if (all.length > 0) {
          ...
          res
            .status(500)
            .json({ error: 'DNS lookup failed', message: 'No records found', hostname });
        }
      } catch {
...
app.post(
  '/api/management/platformmanager/v1/configuration/backup',
  requireAuth,
  jsonParser,
  (req, res) => {
...
app.post(
  '/api/management/platformmanager/v1/configuration/restore',
  requireAuth,
  jsonParser,
  (req, res) => {
...
app.get(
  '/api/management/platformmanager/v1/configuration/download/:filename',
  requireAuth,
  (req, res) => {
...
app.post(
  '/api/management/platformmanager/v1/license/install',
  requireAuth,
  jsonParser,
  (req, res) => {
...
app.post(
  '/api/management/v1/security/rogue-ap/:mac/classify',
  requireAuth,
  jsonParser,
  (req, res) => {
...
app.post('/api/management/platformmanager/v1/startappacketcapture', jsonParser, (req, res) => {
  ...
  setTimeout(
    ...
    Math.min((duration || 60) * 1000, 30000)
  ); // Cap at 30s for simulation
...
function getControllerUrl(req) {
  ...
  if (controllerHeader) {
    try {
      ...
      if ((url.protocol === 'https:' || url.protocol === 'http:') && !isSsrfBlocked(url.hostname)) {
        return normalizeControllerUrl(controllerHeader);
      }
    ...
    xiqRes.on('end', () => {
      ...
      if (xiqRes.statusCode !== 200) {
        const message =
          ...
          body.message ||
          body.error ||
          `XIQ login failed (${xiqRes.statusCode})`;
