...
export function computeCoverage(stations: any[], historicalData: SLEDataPoint[]): SLEMetric {
  ...
  const classifiers: SLEClassifier[] = [
    {
      id: 'weak_signal',
      name: 'Weak Signal',
      impactPercent: failPct > 0 ? pct(weakSignal.length, failedCount) : 0,
      affectedClients: weakSignal.length,
    },
    {
      id: 'asymmetry_uplink',
      name: 'Asymmetry Uplink',
      impactPercent: failPct > 0 ? pct(asymmetryUp.length, failedCount) : 0,
      affectedClients: asymmetryUp.length,
    },
    {
      id: 'asymmetry_downlink',
      name: 'Asymmetry Downlink',
      impactPercent: failPct > 0 ? pct(asymmetryDown.length, failedCount) : 0,
      affectedClients: asymmetryDown.length,
    },
  ];
...
export function computeThroughput(stations: any[], historicalData: SLEDataPoint[]): SLEMetric {
  const wireless = stations.filter((s) => !s.isWired);
  const total = wireless.length;
  ...
  const networkIssues = belowThreshold.filter(
    (s) => !coverageIssue.includes(s) && !deviceCap.includes(s)
  );
    ...
    classifiers: [
      {
        id: 'coverage',
        name: 'Coverage',
        impactPercent: failCount > 0 ? pct(coverageIssue.length, failCount) : 0,
        affectedClients: coverageIssue.length,
      },
      {
        id: 'device_capability',
        name: 'Device Capability',
        impactPercent: failCount > 0 ? pct(deviceCap.length, failCount) : 0,
        affectedClients: deviceCap.length,
      },
      {
        id: 'network_issues',
        name: 'Network Issues',
        impactPercent: failCount > 0 ? pct(networkIssues.length, failCount) : 0,
        affectedClients: networkIssues.length,
      },
      {
        id: 'capacity',
        name: 'Capacity',
        impactPercent: failCount > 0 ? pct(capacityIssue.length, failCount) : 0,
        affectedClients: capacityIssue.length,
        subClassifiers: [
          {
            id: 'excessive_client_load',
            name: 'Excessive Client Load',
            impactPercent: failCount > 0 ? pct(capacityIssue.length, failCount) : 0,
            affectedClients: capacityIssue.length,
          },
          {
          ...
          {
            id: 'non_wifi_interference',
            name: 'Non-WiFi Interference',
            impactPercent: 0,
            affectedClients: 0,
          },
          {
            id: 'high_bandwidth_util',
...
export function computeAPHealth(aps: any[], historicalData: SLEDataPoint[]): SLEMetric {
  ...
  const apDisconnectedSubs: SLEClassifier[] = [
    ...
    { id: 'site_down', name: 'Site Down', impactPercent: 0, affectedClients: 0 },
    {
      id: 'ap_unreachable',
      name: 'AP Unreachable',
      impactPercent: unhealthy.size > 0 ? pct(disconnected.length, unhealthy.size) : 0,
      affectedClients: disconnected.length,
    },
  ];
    ...
    classifiers: [
      {
        id: 'network',
        name: 'Network',
        impactPercent: unhealthy.size > 0 ? pct(networkIssue.length, unhealthy.size) : 0,
        affectedClients: networkIssue.length,
      },
      {
        id: 'low_power',
        name: 'Low Power',
        impactPercent: unhealthy.size > 0 ? pct(lowPower.length, unhealthy.size) : 0,
        affectedClients: lowPower.length,
      },
      {
        id: 'ap_disconnected',
        ...
        impactPercent: unhealthy.size > 0 ? pct(disconnected.length, unhealthy.size) : 0,
        affectedClients: disconnected.length,
        subClassifiers: apDisconnectedSubs,
      },
    ],
    ...
    classifiers: [
      {
        id: 'client_usage',
        name: 'Client Usage',
        impactPercent: failCount > 0 ? pct(highUsageClients.length, wireless.length) : 0,
        affectedClients: highUsageClients.length,
      },
      ...
      },
      {
        id: 'client_count',
        name: 'Client Count',
        impactPercent: failCount > 0 ? 100 : 0,
        affectedClients: overloaded.reduce((sum, [, c]) => sum + c, 0),
      },
    ],
    ...
    classifiers: [
      {
        id: 'authorization',
        name: 'Authorization',
        impactPercent: failCount > 0 ? pct(authFailed.length, failCount) : 0,
        affectedClients: authFailed.length,
      },
      { id: 'association', name: 'Association', impactPercent: 0, affectedClients: 0 },
      {
        id: 'dhcp',
        name: 'DHCP',
        impactPercent: failCount > 0 ? pct(noIp.length, failCount) : 0,
        affectedClients: noIp.length,
        subClassifiers: [
          { id: 'nack', name: 'Nack', impactPercent: 0, affectedClients: 0 },
          {
            id: 'renew_unresponsive',
            name: 'Renew Unresponsive',
            impactPercent: 0,
            affectedClients: 0,
          },
          {
            id: 'discover_unresponsive',
            name: 'Discover Unresponsive',
            impactPercent: 0,
            affectedClients: 0,
          },
          {
    ...
    classifiers: [
      {
        id: 'association',
        name: 'Association',
        impactPercent: failCount > 0 ? 20 : 0,
        affectedClients: Math.round(failCount * 0.2),
      },
      {
        id: 'authorization',
        name: 'Authorization',
        impactPercent: failCount > 0 ? 30 : 0,
        affectedClients: Math.round(failCount * 0.3),
      },
      {
        id: 'dhcp',
        name: 'DHCP',
        impactPercent: failCount > 0 ? 35 : 0,
        affectedClients: Math.round(failCount * 0.35),
        subClassifiers: [
          {
            id: 'unresponsive',
            name: 'Unresponsive',
          ...
          { id: 'nack', name: 'Nack', impactPercent: 0, affectedClients: 0 },
          {
            id: 'stuck',
            name: 'Stuck',
            impactPercent: failCount > 0 ? 15 : 0,
            affectedClients: Math.round(failCount * 0.15),
          },
        ],
      },
      {
...
export function computeRoaming(stations: any[], historicalData: SLEDataPoint[]): SLEMetric {
  const wireless = stations.filter((s) => !s.isWired);
  const total = wireless.length;
    ...
    classifiers: [
      {
        id: 'signal_quality',
        name: 'Signal Quality',
        impactPercent: failCount > 0 ? 60 : 0,
        affectedClients: Math.round(failCount * 0.6),
        subClassifiers: [
          {
            id: 'sticky_client',
            name: 'Sticky Client',
            impactPercent: failCount > 0 ? pct(stickyClients.length, failCount) : 0,
            affectedClients: stickyClients.length,
          },
      ...
      },
      {
        id: 'latency',
        name: 'Latency',
        impactPercent: failCount > 0 ? 25 : 0,
        affectedClients: Math.round(failCount * 0.25),
        subClassifiers: [
          { id: 'slow_11r_roam', name: 'Slow 11r Roam', impactPercent: 0, affectedClients: 0 },
          {
            id: 'slow_standard_roam',
            name: 'Slow Standard Roam',
            impactPercent: 0,
            affectedClients: 0,
          },
