...
import { type EnvironmentProfile } from '../config/environmentProfiles';
...
export function generateInsights(
  ...
  _existingEvents?: Array<{ type: string; timestamp: number; message: string }>
): InsightCard[] {
  ...
  if (metrics.rfqi !== undefined && metrics.rfqi < thresholds.rfqiPoor) {
    const severity: InsightSeverity =
      metrics.rfqi < thresholds.rfqiPoor * 0.7 ? 'critical' : 'warning';
    insights.push({
      id: `rfqi-low-${now}`,
      ...
      recommendedAction:
        'Review RF environment for interference sources. Consider channel optimization or AP power adjustments.',
      category: 'rf_quality',
  ...
  if (
    ...
    metrics.channelUtilization > thresholds.channelUtilizationPct
  ) {
    insights.push({
      ...
      recommendedAction:
        'Consider load balancing clients across APs or adding capacity in high-density areas.',
      ...
      impact:
        (metrics.channelUtilization - thresholds.channelUtilizationPct) /
        (100 - thresholds.channelUtilizationPct),
      confidence: 0.85,
      recurrence: 0.6,
      rankScore: 0,
      createdAt: now,
  ...
  if (metrics.interference !== undefined && metrics.interference > thresholds.interferenceHigh) {
    insights.push({
      ...
      evidence: [
        {
          ...
          value: `${(metrics.interference * 100).toFixed(1)}%`,
          metric: 'interference',
          timestamp: metrics.timestamp,
        },
      ...
      recommendedAction:
        'Identify interference sources (microwaves, Bluetooth, neighboring networks). Consider dynamic channel selection.',
  ...
  if (metrics.retryRate !== undefined && metrics.retryRate > thresholds.retryRatePct) {
    insights.push({
      ...
      recommendedAction:
        'Check for co-channel interference, adjust AP transmit power, or relocate affected clients.',
      category: 'rf_quality',
      group: 'network_health',
      severity: 'warning',
      scope: 'AP',
  ...
  if (metrics.apCount !== undefined && metrics.apOnlineCount !== undefined) {
    ...
    if (offlineCount > 0 && offlinePercent > 5) {
      insights.push({
        ...
        recommendedAction:
          'Check network connectivity to offline APs. Verify power and physical connections.',
  ...
  if (
    ...
    if (clientsPerAP > thresholds.clientDensity * 1.2) {
      insights.push({
        ...
        recommendedAction:
          'Consider adding access points to high-density areas or enabling band steering.',
        category: 'capacity',
  ...
  if (metrics.avgRssi !== undefined && metrics.avgRssi < -75) {
    insights.push({
      ...
      recommendedAction:
        'Review AP placement. Clients may be too far from access points or experiencing physical obstructions.',
  ...
  if (metrics.rfqi !== undefined && metrics.history?.rfqi1hAgo !== undefined) {
    ...
    if (Math.abs(changePercent) > 10) {
      ...
      const severity: InsightSeverity =
        direction === 'degrading' && changePercent < -20 ? 'warning' : 'info';
  ...
  if (metrics.rfqi !== undefined && metrics.history?.rfqi24hAgo !== undefined) {
    ...
    if (changePercent < -15) {
      insights.push({
        ...
        recommendedAction:
          'Compare environmental conditions. Check for patterns (e.g., peak hours, external interference).',
        category: 'historical',
        group: 'anomaly_detection',
        severity: 'info',
        scope: 'SITE',
  ...
  if (metrics.clientCount !== undefined && metrics.history?.clientCount1hAgo !== undefined) {
    ...
    const changePercent =
      metrics.history.clientCount1hAgo > 0 ? (change / metrics.history.clientCount1hAgo) * 100 : 0;
    ...
    if (changePercent > 50 && change > 10) {
      insights.push({
        ...
        recommendedAction:
          'Monitor for capacity issues. Ensure load balancing is active and consider temporary capacity measures.',
        category: 'anomaly',
        group: 'anomaly_detection',
  ...
  if (metrics.apMetrics && metrics.apMetrics.length > 0) {
    const apsWithHighRestarts = metrics.apMetrics.filter(
      (ap) =>
        (ap.restartCount || 0) >= 3 && ap.lastRestart && now - ap.lastRestart < 24 * 60 * 60 * 1000
    );
    ...
    if (apsWithHighRestarts.length > 0) {
      insights.push({
        ...
        whyItMatters:
          'Multiple restarts in 24 hours indicate potential hardware issues or firmware instability.',
        evidence: [
          { label: 'Affected APs', value: apsWithHighRestarts.length },
          {
            label: 'AP Names',
            value: apsWithHighRestarts
              .slice(0, 3)
              .map((ap) => ap.name)
              .join(', '),
          },
          {
            label: 'Avg Restarts',
            value: (
              apsWithHighRestarts.reduce((sum, ap) => sum + (ap.restartCount || 0), 0) /
              apsWithHighRestarts.length
            ).toFixed(1),
          },
        ],
        recommendedAction:
          'Schedule maintenance check. Consider firmware update or hardware replacement for persistently unstable APs.',
        category: 'predictive',
  ...
  if (metrics.apMetrics && metrics.apMetrics.length > 0) {
    const stressedAPs = metrics.apMetrics.filter(
      (ap) =>
    ...
    if (stressedAPs.length > 0) {
      insights.push({
        ...
        whyItMatters:
          'High memory or CPU usage may lead to performance degradation or unexpected reboots.',
        evidence: [
          ...
          {
            label: 'Avg Memory',
            value: `${(stressedAPs.reduce((sum, ap) => sum + (ap.memoryUsage || 0), 0) / stressedAPs.length).toFixed(0)}%`,
          },
        ],
        recommendedAction:
          'Review client distribution. Consider offloading clients to nearby APs or investigate memory leaks.',
        category: 'predictive',
        group: 'predictive_maintenance',
        severity: 'warning',
        scope: 'AP',
  ...
  if (
    ...
    const clientsPerAP = metrics.clientCount / metrics.apOnlineCount;
    const growthRate =
      metrics.history.clientCount24hAgo > 0
        ? (metrics.clientCount - metrics.history.clientCount24hAgo) /
          metrics.history.clientCount24hAgo
        : 0;
    ...
    if (growthRate > 0.1 && clientsPerAP > thresholds.clientDensity * 0.8) {
      ...
      const daysToCapacity =
        clientsPerAP < thresholds.clientDensity
      ...
      insights.push({
        ...
        recommendedAction:
          'Plan for additional AP deployment. Review high-density areas and consider load balancing optimizations.',
        category: 'predictive',
        group: 'capacity_planning',
...
export function calculateAIBaseline(input: AIBaselineInput): AIBaselineThresholds {
  ...
  const retryP85 =
    input.retryRateHistory && input.retryRateHistory.length > 0
      ? percentile(input.retryRateHistory, 85)
      : 15; // Default

  // Latency P95
  const latencyP95 =
    input.latencyHistory && input.latencyHistory.length > 0
