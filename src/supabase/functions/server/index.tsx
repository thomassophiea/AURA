...
app.post('/make-server-efba0687/kv/mset', async (c) => {
  try {
    ...
    if (!keys || !values || keys.length !== values.length) {
      return c.json(
        { error: 'Keys and values arrays are required and must be the same length' },
        400
      );
    }
...
app.post('/make-server-efba0687/throughput/snapshot', async (c) => {
  try {
    ...
    if (
      !snapshot.timestamp ||
      typeof snapshot.totalUpload !== 'number' ||
      typeof snapshot.totalDownload !== 'number'
    ) {
      return c.json(
        { error: 'Invalid snapshot data. Required fields: timestamp, totalUpload, totalDownload' },
        400
      );
    }
...
app.get('/make-server-efba0687/throughput/network/:networkName', async (c) => {
  try {
    ...
    const trends = await throughputTracker.getNetworkThroughputTrends(
      networkName,
      startTime,
      endTime
    );
