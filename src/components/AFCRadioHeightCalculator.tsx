...
export function AFCRadioHeightCalculator() {
  ...
  const [buildingCeilingHelpers, setBuildingCeilingHelpers] = useState<Map<string, number>>(
    new Map()
  );
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [aps, savedHeights] = await Promise.allSettled([
        ...
        apiService
          .makeAuthenticatedRequest('/v1/afc/radio-heights')
          .then((r) => (r.ok ? r.json() : []))
          .catch(() => []),
      ]);
      ...
      floors.forEach((fc) => {
        if (!seenS.has(fc.site)) {
          seenS.add(fc.site);
          expanded.add(`site:${fc.site}`);
        }
      ...
      const records: APHeightRecord[] = apList.map((ap) => {
        ...
        const apH =
          typeof s.apHeightAboveFloor === 'number'
            ? s.apHeightAboveFloor
            : (fc?.apHeightDefault ?? 3.0);
        const apU =
          typeof s.apHeightUncertainty === 'number'
            ? s.apHeightUncertainty
        ...
        return {
          ...
          floorHeightOverride:
            typeof s.floorHeightOverride === 'number' ? s.floorHeightOverride : null,
          floorUncertaintyOverride:
            typeof s.floorUncertaintyOverride === 'number' ? s.floorUncertaintyOverride : null,
          deployment:
            typeof s.deployment === 'string' ? (s.deployment as Deployment) : autoDeployment,
  ...
  const unsetFloors = useMemo(
    () =>
      floorConfigs.filter((fc) => {
        ...
        return apRecords.some(
          (r) =>
            r.site === fc.site &&
            r.building === fc.building &&
            r.floor === fc.floor &&
            r.powerClass === 'sp'
        );
      }),
  ...
  const toggleAll = () => {
    ...
    setApRecords((prev) =>
      prev.map((r) => (sns.has(r.serialNumber) ? { ...r, selected: next } : r))
    );
  };
  const toggleOne = (sn: string) =>
    setApRecords((prev) =>
      prev.map((r) => (r.serialNumber === sn ? { ...r, selected: !r.selected } : r))
    );
  const toggleSite = (site: string) => {
    ...
    setApRecords((prev) =>
      prev.map((r) => (sns.has(r.serialNumber) ? { ...r, selected: !all } : r))
    );
  };
      ...
      prev.map((r) =>
        r.serialNumber !== sn
          ...
          : {
              ...r,
              deployment: d,
              powerClass: d === 'outdoor' && r.powerClass === 'lp' ? 'sp' : r.powerClass,
            }
      )
    );
  ...
  const bulkSetPowerClass = (pc: PowerClass) =>
    setApRecords((prev) =>
      prev.map((r) =>
        !r.selected || (pc === 'lp' && r.deployment === 'outdoor') ? r : { ...r, powerClass: pc }
      )
    );
  ...
  const applyFloorFormula = (site: string, building: string, ceilingHeight: number) => {
    setFloorConfigs((prev) =>
      prev.map((fc) => {
        ...
        const h =
          fc.floorNumber > 0 && fc.floorNumber <= 20 ? (fc.floorNumber - 1) * ceilingHeight : 0;
  ...
  const handleCsvImport = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (ev) => {
      ...
      if (!rows.length) {
        toast.error('No valid rows. Expected: Building,Floor Label,Floor Height Above Ground (m)');
        return;
      }
      let updated = 0;
      setFloorConfigs((prev) => {
        ...
        rows.forEach((row) => {
          ...
          if (idx >= 0) {
            next[idx] = { ...next[idx], floorHeightAboveGround: row.height };
            updated++;
          }
        });
        return next;
  ...
  const applyRecords = async (toApply: APHeightRecord[]) => {
    ...
    for (let i = 0; i < toApply.length; i += BATCH) {
      ...
      try {
        ...
        setApplyStatus((prev) => {
          const n = new Map(prev);
          batch.forEach((r) => n.set(r.serialNumber, 'applied'));
          return n;
        });
        ...
        setApplyStatus((prev) => {
          const n = new Map(prev);
          batch.forEach((r) => n.set(r.serialNumber, 'error'));
          return n;
        });
        failed += batch.length;
      }
  ...
  const saveProfile = () => {
    ...
    const profile: ConfigProfile = {
      ...
      defaultApHeight,
      defaultFloorUncertainty,
      defaultApUncertainty,
      defaultAntennaType,
      defaultDeployment,
      defaultPowerClass,
      siteElevations: Object.fromEntries(siteElevations),
      floorOverrides: Object.fromEntries(
        floorConfigs
          .filter((fc) => fc.floorHeightAboveGround !== 0)
          .map((fc) => [fc.key, fc.floorHeightAboveGround])
      ),
    };
    ...
    <div className="space-y-4 p-6">
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">
            Set floor and AP heights for AFC Standard Power compliance — enter heights directly as
            shown in Professional Install
          </p>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <Button
            ...
            disabled={applying || !filteredRecords.length}
            className="gap-1.5"
          >
            {applying && applyProgress ? (
              <>
                <RefreshCw className="h-3.5 w-3.5 animate-spin" />
                Saving {applyProgress.current}/{applyProgress.total}…
              </>
            ) : (
              <>
                <CheckCheck className="h-4 w-4" />
                Apply All ({filteredRecords.length})
              </>
            )}
          </Button>
        ...
        <Card className="border-violet-500/40 shadow-lg shadow-violet-500/10 overflow-hidden">
          ...
          <div className="bg-gradient-to-r from-violet-600 to-indigo-600 px-4 py-3 flex items-center justify-between">
            ...
            <Button
              ...
              onClick={() => setShowCampusLookup(false)}
            >
              <X className="h-3.5 w-3.5" />
            </Button>
          </div>
          ...
          <CardContent className="pt-4 space-y-4">
            <p className="text-xs text-muted-foreground">
              Search publicly available campus building records to auto-populate floor heights.
              Results are estimates based on architectural specifications and are for demonstration
              purposes only.
            </p>
              ...
              <div className="flex flex-col items-center gap-3 py-6">
                ...
                <div className="flex gap-1 mt-1">
                  {CAMPUS_SEARCH_STEPS.map((_, i) => (
                    <div
                      key={i}
                      className={`h-1 w-8 rounded-full transition-all duration-300 ${i <= campusSearchStep ? 'bg-violet-500' : 'bg-muted'}`}
                    />
                  ))}
                </div>
              </div>
      ...
      <Card>
        <CardHeader className="pb-3">
          ...
          <CardDescription>
            Applied when no per-site, per-floor, or per-AP override is set
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-4">
            ...
            <div className="space-y-1.5 col-span-2 md:col-span-1">
              ...
              <Input
                placeholder="e.g. Internal_5050D"
                value={defaultAntennaType}
            ...
            <div className="space-y-1.5">
              ...
              <Select
                ...
                onValueChange={(v) => setDefaultDeployment(v as Deployment)}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
            ...
            <div className="space-y-1.5">
              ...
              <Select
                value={defaultPowerClass}
                onValueChange={(v) => setDefaultPowerClass(v as PowerClass)}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
        ...
        <TabsList>
          ...
          <TabsTrigger value="floors" className="flex items-center gap-1.5">
            ...
            {unsetFloors.length > 0 && (
              <Badge className="ml-1 text-xs bg-[color:color-mix(in_oklab,var(--warning)_15%,transparent)] text-warning border-[color:var(--warning)]/30">
                {unsetFloors.length} unset
              </Badge>
            )}
          </TabsTrigger>
        ...
        <TabsContent value="aps" className="mt-4 space-y-3">
          ...
          {unsetFloors.length > 0 && (
            <Alert className="border-[color:var(--status-warning)]/50 bg-[color:var(--status-warning-bg)]">
              ...
              <AlertDescription className="text-sm">
                <strong>
                  {unsetFloors.length} floor{unsetFloors.length !== 1 ? 's' : ''}
                </strong>{' '}
                with SP APs still have Floor Height Above Ground = 0. Go to the{' '}
                <strong>Floor Heights</strong> tab and enter the correct values.
              </AlertDescription>
            </Alert>
          )}
          {validationWarnings.length > 0 && (
            <Alert className="border-[color:var(--status-error)]/50 bg-[color:var(--status-error-bg)]">
              ...
              <AlertDescription className="text-sm">
                <strong>
                  {validationWarnings.length} AP{validationWarnings.length !== 1 ? 's' : ''}
                </strong>{' '}
                have total heights outside 0–150 m — verify floor height settings.
              </AlertDescription>
            </Alert>
          )}
              ...
              {filteredRecords.length === 0 ? (
                ...
                <div className="overflow-x-auto rounded-md border">
                  <Table>
                    ...
                    <TableBody>
                      {(
                        Array.from(hierarchy.entries()) as [
                          string,
                          Map<string, Map<string, APHeightRecord[]>>,
                        ][]
                      ).map(([site, bMap]) => {
                          ...
                          <>
                            <TableRow key={siteKey} className="bg-muted/40 hover:bg-muted/50">
                              <TableCell>
                                <Checkbox
                                  checked={
                                    siteRecs.every((r) => r.selected)
                                      ? true
                                      : siteRecs.some((r) => r.selected)
                                        ? 'indeterminate'
                                        : false
                                  }
                              ...
                              <TableCell colSpan={9}>
                                <div className="flex items-center gap-2 flex-wrap">
                                  ...
                                  <div className="flex items-center gap-1.5 ml-2">
                                    ...
                                    {siteElev !== undefined && (
                                      <button
                                        onClick={() => updateSiteElevation(site, '')}
                            ...
                            {siteExpanded &&
                              (
                                Array.from(bMap.entries()) as [
                                  string,
                                  Map<string, APHeightRecord[]>,
                                ][]
                                  ...
                                  <>
                                    <TableRow key={bKey} className="bg-muted/20 hover:bg-muted/30">
                                      <TableCell>
                                        <Checkbox
                                          checked={
                                            bRecs.every((r) => r.selected)
                                              ? true
                                              : bRecs.some((r) => r.selected)
                                                ? 'indeterminate'
                                                : false
                                          }
                                      ...
                                      <TableCell colSpan={10}>
                                        <div className="flex items-center gap-2 pl-4">
                                          <button
                                            onClick={() => toggleExpanded(bKey)}
                                            className="flex items-center gap-1.5 font-medium text-sm hover:text-primary"
                                          >
                                            {bExpanded ? (
                                              <ChevronDown className="h-3.5 w-3.5" />
                                            ) : (
                                    ...
                                    {bExpanded &&
                                      (
                                        Array.from(floorMap.entries()) as [
                                          string,
                                          APHeightRecord[],
                                        ][]
                                      ).map(([floor, floorAps]) => {
                                        ...
                                        const floorHAG =
                                          (siteElevations.get(site) ?? defaultGroundElevation) +
                                          (fc?.floorHeightAboveGround ?? 0);
                                        ...
                                        return (
                                          <>
                                              ...
                                              <TableCell colSpan={10}>
                                                <div className="flex items-center gap-2 pl-8 text-xs text-muted-foreground">
                                                  ...
                                                  <span
                                                    className={
                                                      fc?.floorHeightAboveGround === 0 &&
                                                      floorAps.some((a) => a.powerClass === 'sp')
                                                        ? 'text-[color:var(--status-warning)] font-medium'
                                                        : ''
                                                    }
                                                  >
                                            ...
                                            {floorAps.map((ap) => {
                                              const total =
                                                ap.floorHeightAboveGround + ap.apHeightAboveFloor;
                                              const isWarn = total < 0 || total > 150;
                                              const status =
                                                applyStatus.get(ap.serialNumber) ?? 'idle';
                                                ...
                                                >
                                                  ...
                                                  <TableCell>
                                                    {isLp ? (
                                                      ...
                                                      <Input
                                                        value={ap.antennaType}
                                                        placeholder={
                                                          defaultAntennaType || 'Antenna type…'
                                                        }
                                                        onChange={(e) =>
                                                          updateAP(ap.serialNumber, {
                                                            antennaType: e.target.value,
                                                          })
                                                        }
                                                  ...
                                                  <TableCell className="text-right">
                                                    {ap.floorSettingsOverride ? (
                                                      <Input
                                                        ...
                                                        step="0.1"
                                                        value={
                                                          ap.floorHeightOverride ??
                                                          ap.floorHeightAboveGround
                                                        }
                                                        onChange={(e) =>
                                                          updateAP(ap.serialNumber, {
                                                            floorHeightOverride:
                                                      ...
                                                      <span className="font-mono text-sm">
                                                        {ap.floorHeightAboveGround.toFixed(1)}
                                                      </span>
                                                    )}
                                                  </TableCell>
                                                  <TableCell className="text-right">
                                                    {ap.floorSettingsOverride ? (
                                                      <Input
                                                        ...
                                                        min="0"
                                                        value={
                                                          ap.floorUncertaintyOverride ??
                                                          ap.floorHeightUncertainty
                                                        }
                                                        onChange={(e) =>
                                                          updateAP(ap.serialNumber, {
                                                            floorUncertaintyOverride:
                                                      ...
                                                      <span className="font-mono text-sm text-muted-foreground">
                                                        {ap.floorHeightUncertainty.toFixed(1)}
                                                      </span>
                                                    )}
                                                  </TableCell>
                                                  <TableCell className="text-right">
                                                  ...
                                                  <TableCell className="text-right">
                                                    <span
                                                      className={`font-mono font-semibold text-sm ${isWarn ? 'text-[color:var(--status-error)]' : ''}`}
                                                    >
                                                      {total.toFixed(1)}
                                                      {isWarn && (
                                                  ...
                                                  <TableCell>
                                                    ...
                                                    >
                                                      ...
                                                      <SelectContent>
                                                        ...
                                                        <SelectItem
                                                          value="lp"
                                                          disabled={ap.deployment === 'outdoor'}
                                                        >
                                                          LP
                                                        </SelectItem>
                                                      </SelectContent>
                                                  ...
                                                  <TableCell className="text-center">
                                                    <div className="flex flex-col items-center gap-1">
                                                      ...
                                                      <label
                                                        ...
                                                        title="Override floor heights for this AP"
                                                      >
                                                        <Checkbox
                                                          checked={ap.floorSettingsOverride}
                                                          className="h-3.5 w-3.5"
                                                          onCheckedChange={(v) =>
                                                            updateAP(ap.serialNumber, {
                                                              floorSettingsOverride: !!v,
                                                              floorHeightOverride: v
                                                                ? ap.floorHeightAboveGround
                                                                : null,
                                                              floorUncertaintyOverride: v
                                                                ? ap.floorHeightUncertainty
                                                                : null,
                                                            })
                                                          }
          ...
          <Card>
            ...
            <CardContent className="space-y-6">
              {!floorConfigs.length ? (
                <Alert>
                  <AlertCircle className="h-4 w-4" />
                  <AlertDescription>No floors found.</AlertDescription>
                </Alert>
              ) : (
                <>
                  <p className="text-xs text-muted-foreground">
                    ...
                    <code className="bg-muted px-1 rounded">
                      Building,Floor Label,Floor Height Above Ground (m)
                    </code>
                  </p>
                  {([...new Set(floorConfigs.map((fc) => fc.site))] as string[])
                    .sort()
                    .map((site: string) => (
                      <div key={site} className="space-y-4">
                        ...
                        {(
                          [
                            ...new Set(
                              floorConfigs.filter((fc) => fc.site === site).map((fc) => fc.building)
                            ),
                          ] as string[]
                        )
                          .sort()
                          .map((building: string) => {
                            const bFloors = floorConfigs.filter(
                              (fc) => fc.site === site && fc.building === building
                            );
                                  ...
                                  <div className="flex items-center gap-2 flex-wrap">
                                    ...
                                    <div className="flex items-center gap-1.5 border rounded-md px-2 py-1 bg-background">
                                      ...
                                      <Input
                                        ...
                                        value={helperCeiling}
                                        onChange={(e) =>
                                          setBuildingCeilingHelpers((prev) => {
                                            const n = new Map(prev);
                                            n.set(bKey, parseFloat(e.target.value) || 3.5);
                                            return n;
                                          })
                                        }
                                      ...
                                      <Button
                                        ...
                                        onClick={() =>
                                          applyFloorFormula(site, building, helperCeiling)
                                        }
                                      >
                                        Calculate
                                      </Button>
                                    ...
                                    {otherBuildings.length > 0 && (
                                      <div className="relative">
                                        <Button
                                          ...
                                          onClick={() =>
                                            setCopySource(copySource === bKey ? null : bKey)
                                          }
                                        >
                                    ...
                                    {bFloors.map((fc) => {
                                      ...
                                      return (
                                        <TableRow
                                          key={fc.key}
                                          className={
                                            needsEntry ? 'bg-[color:var(--status-warning-bg)]' : ''
                                          }
                                        >
                                          <TableCell>
                                            <div className="flex items-center gap-1.5">
                                              ...
                                              {needsEntry && (
                                                <span title="SP AP — height not set">
            ...
            <CardContent className="space-y-6">
              <Table>
                ...
                <TableBody>
                  <TableRow>
                    <TableCell>
                      <Badge className="bg-[color:var(--status-info-bg)] text-[color:var(--status-info)] border-[color:var(--status-info)]/20 border">
                        Standard Power (SP)
                      </Badge>
                    </TableCell>
              ...
              <Alert>
                ...
                <AlertDescription className="text-sm space-y-1">
                  ...
                  <p>
                    Total Radio Height submitted to AFC = Floor Height Above Ground + AP Height
                    Above Floor.
                  </p>
                </AlertDescription>
              </Alert>
