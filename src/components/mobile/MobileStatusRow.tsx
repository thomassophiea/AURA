    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2">
          <span className="text-sm font-medium text-foreground truncate">{primaryText}</span>
          {status && (
        ...
        <p className="text-xs text-muted-foreground/80 truncate mt-0.5">{secondaryText}</p>
      </div>

      {/* Right Content or Chevron */}
      {rightContent ||
        (onClick && <ChevronRight className="h-4 w-4 text-muted-foreground/60 flex-shrink-0" />)}
    </Component>
  );
}
