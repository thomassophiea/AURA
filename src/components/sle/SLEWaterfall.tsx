  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex items-center gap-3 px-2 pb-1 border-b border-border/30">
        <span className="text-xs font-semibold text-muted-foreground uppercase tracking-widest w-36 shrink-0">
          Metric
        </span>
        <span className="text-xs font-semibold text-muted-foreground uppercase tracking-widest flex-1">
          Performance
        </span>
        <span className="text-xs font-semibold text-muted-foreground uppercase tracking-widest w-16 text-right">
          Score
        </span>
      </div>
          ...
          <div key={sle.id}>
            ...
            >
              ...
              <div className="flex-1 relative h-8 rounded overflow-hidden bg-muted/30">
                ...
                {failPct > 0.5 &&
                  activeClassifiers.map((c, i) => {
                    const segW = (c.impactPercent / totalImpact) * failPct;
                    const prevW = activeClassifiers
                      .slice(0, i)
                      .reduce((acc, pc) => acc + (pc.impactPercent / totalImpact) * failPct, 0);
                ...
                <div className="absolute inset-0 flex items-center px-2">
                  <span className="text-xs font-medium text-muted-foreground truncate">
                    {activeClassifiers.length > 0
                      ? activeClassifiers.map((c) => c.name).join(' · ')
                      : 'No issues detected'}
                  </span>
                </div>
              </div>
            ...
            {isSelected && (
              <div
                ...
                style={{
                  background: STATUS_DETAIL_BG[sle.status],
                  border: `1px solid ${STATUS_NODE_BORDER[sle.status]}`,
                }}
              >
                <div className="flex items-center gap-2.5 px-5 pt-4 pb-2">
                  <div>
                    ...
                    <p className="text-xs text-white/60">{sle.description}</p>
                  </div>
