...
function buildRootCause(
  ...
  if (sle.id === 'coverage' && classifier.id === 'weak_signal') {
    affectedDevices = stations
      ...
      .map((s) => ({
        mac: s.macAddress || '',
        name: s.hostName || s.hostname || s.username || s.deviceType || s.macAddress || 'Unknown',
        ap:
          s.apName ||
          s.apDisplayName ||
          s.apHostname ||
...
export function SLEConstellation({ sles, stations, aps }: SLEConstellationProps) {
  ...
  const bgStarList = useMemo(
    () => makeBgStars(80, containerW, containerH),
        ...
        >
          <defs>
            ...
            <filter id="cBodyGlow" x="-80%" y="-80%" width="260%" height="260%">
              ...
              <feFlood
                floodColor={SLE_STATUS_COLORS[overallStatus].hex}
                floodOpacity="0.3"
                result="c"
              />
              <feComposite in="c" in2="blur" operator="in" result="g" />
          ...
          <text
            ...
            fontSize={14}
            fontWeight="bold"
            fill={SLE_STATUS_COLORS[overallStatus].hex}
          >
              ...
              >
                ...
                <path
                  d={starPath(
                    x,
                    y,
                    active ? outerR * 1.25 : outerR,
                    active ? innerR * 1.25 : innerR
                  )}
                  fill={color}
