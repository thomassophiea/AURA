...
const METRIC_ICONS: Record<string, React.ComponentType<{ className?: string }>> = {
  _metric_aps: Wifi,
  _metric_total_aps: Wifi,
  _metric_online_aps: CheckCircle,
  _metric_offline_aps: XCircle,
  _metric_clients: Users,
  _metric_total_clients: Users,
  _metric_authenticated: Shield,
  _metric_active_clients: Users,
  _metric_throughput: Zap,
  _metric_total_throughput: Zap,
  _metric_upload: ArrowUpRight,
  _metric_download: ArrowDownRight,
  _metric_health: Shield,
  _metric_sites: Building2,
  _metric_total_sites: Building2,
  _metric_networks: Network,
  _metric_client_networks: Network,
  _metric_avg_rssi: Signal,
  _metric_avg_signal: Signal,
  _metric_ap_models: Router,
  _metric_band_24: Radio,
  _metric_band_5: Radio,
  _metric_band_6: Radio,
  ...
  return (
    <Card>
      <CardContent className="py-8 text-center text-xs text-muted-foreground">
        Unknown widget type: {displayType}
      </CardContent>
    </Card>
  );
}
...
function renderScorecard(widget: ReportWidgetConfig, m: ReportMetrics) {
  ...
  switch (key) {
    case '_metric_aps':
      value = `${m.onlineAps}/${m.totalAps}`;
      sub = `${m.offlineAps} offline`;
      break;
    case '_metric_total_aps':
      value = m.totalAps.toString();
      break;
    case '_metric_online_aps':
      value = m.onlineAps.toString();
      sub = `${((m.onlineAps / Math.max(m.totalAps, 1)) * 100).toFixed(0)}%`;
      break;
    case '_metric_offline_aps':
      value = m.offlineAps.toString();
      break;
    case '_metric_clients':
    case '_metric_total_clients':
    case '_metric_active_clients':
      value = fmtNum(m.totalClients);
      sub = `${m.authenticated} authenticated`;
      break;
    case '_metric_authenticated':
      value = fmtNum(m.authenticated);
      sub = `${m.totalClients > 0 ? ((m.authenticated / m.totalClients) * 100).toFixed(0) : 0}%`;
      break;
    case '_metric_throughput':
    case '_metric_total_throughput':
      value = formatBitsPerSecond(m.totalThroughput);
      sub = `${formatBitsPerSecond(m.totalUpload)} up / ${formatBitsPerSecond(m.totalDownload)} down`;
      break;
    case '_metric_upload':
      value = formatBitsPerSecond(m.totalUpload);
      break;
    case '_metric_download':
      value = formatBitsPerSecond(m.totalDownload);
      break;
    case '_metric_health':
      value = `${m.bpScore}%`;
      sub = `${m.bpGood} pass / ${m.bpWarn + m.bpError} issues`;
      break;
    case '_metric_sites':
    case '_metric_total_sites':
      value = m.totalSites.toString();
      break;
    case '_metric_networks':
    case '_metric_client_networks':
      value = m.totalServices.toString();
      break;
    case '_metric_avg_rssi':
    case '_metric_avg_signal':
      value = `${m.avgRssi} dBm`;
      break;
    case '_metric_ap_models':
      value = m.apModels.length.toString();
      break;
    case '_metric_bp_summary':
      value = `${m.bpGood}/${m.bpTotal}`;
      sub = `${m.bpWarn} warnings, ${m.bpError} errors`;
      break;
    case '_metric_band_24':
      value = (m.bands['2.4 GHz'] || 0).toString();
      break;
    case '_metric_band_5':
      value = (m.bands['5 GHz'] || 0).toString();
      break;
    case '_metric_band_6':
      value = (m.bands['6 GHz'] || 0).toString();
      break;
  }
...
function renderTimeseries(
  ...
  const chartData =
    stats[0]?.values?.map((v: any, idx: number) => {
      const point: any = {
            ...
            <div key={i} className="space-y-1">
              <div className="flex items-center justify-between text-xs">
                ...
                <span className="text-muted-foreground font-mono">
                  {unit === 'bps'
                    ? formatBitsPerSecond(item.value)
                    : unit === 'bytes'
...
function renderComputedRanking(widget: ReportWidgetConfig, m: ReportMetrics) {
  ...
  if (key === '_metric_best_practices_full') {
    return (
      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="text-sm">{widget.title || 'Best Practices'}</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="space-y-2">
  ...
  if (key === '_metric_ap_inventory') {
    return (
      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="text-sm">{widget.title || 'AP Inventory'}</CardTitle>
        </CardHeader>
        <CardContent>
...
function renderBarChart(widget: ReportWidgetConfig, m: ReportMetrics) {
  ...
  if (key === '_metric_ssid_distribution') {
    ...
    return (
      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="text-sm">{widget.title || 'Clients by Network'}</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="space-y-2">
            {data.map((s, i) => (
  ...
  if (key === '_metric_rssi_distribution') {
    ...
    return (
      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="text-sm">{widget.title || 'Signal Quality'}</CardTitle>
        </CardHeader>
        <CardContent>
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={data}>
  ...
  if (key === '_metric_ap_model_distribution') {
    ...
    return (
      <Card>
        <CardHeader className="pb-2">
          <CardTitle className="text-sm">{widget.title || 'AP Model Distribution'}</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="space-y-2">
            {data.map((d, i) => {
