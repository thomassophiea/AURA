...
export function IotEditor({
        ...
        <Group title={root === 'genericScan' ? 'Scan Parameters' : 'Scan'}>
          <FieldRow label="Scan Interval (ms)" error={errs[`${root}.interval`]} required>
            <Input
              type="number"
              disabled={ro}
              value={Number.isFinite(s.interval) ? s.interval : ''}
              onChange={(e) => updSub(root, { interval: numVal(e.target.value) })}
          ...
          <FieldRow label="Scan Window (ms)" error={errs[`${root}.window`]} required>
            <Input
              type="number"
              disabled={ro}
              value={Number.isFinite(s.window) ? s.window : ''}
              onChange={(e) => updSub(root, { window: numVal(e.target.value) })}
              className="max-w-[160px]"
            />
          </FieldRow>
        </Group>
        <Group title="Filter">
          {hasUuid && (
            <FieldRow label="UUID" error={errs[`${root}.uuid`]} required>
              <Input
                disabled={ro}
                value={(s as IBeaconScan).uuid ?? ''}
                onChange={(e) => updSub(root, { uuid: e.target.value } as Partial<IBeaconScan>)}
          ...
          <FieldRow label="Min RSS (dBm)" error={errs[`${root}.minRSS`]} required>
            <Input
              type="number"
              disabled={ro}
              value={Number.isFinite(s.minRSS) ? s.minRSS : ''}
              onChange={(e) => updSub(root, { minRSS: numVal(e.target.value) })}
              className="max-w-[160px]"
            />
          </FieldRow>
          ...
          {setFwd && (
            <FieldRow label="Forward to external server" inline>
              <Checkbox
                checked={fwd}
                disabled={ro}
                onCheckedChange={(v) => setFwd(v === true)}
                aria-label="Forward to external server"
              />
            </FieldRow>
          )}
          {destShown && (
            <>
              <FieldRow label="Destination IP" error={errs[`${root}.destAddr`]} required>
                <Input
                  disabled={ro}
                  value={s.destAddr ?? ''}
                  onChange={(e) => updSub(root, { destAddr: e.target.value })}
              ...
              <FieldRow label="Destination Port" error={errs[`${root}.destPort`]} required>
                <Input
                  type="number"
                  disabled={ro}
                  value={Number.isFinite(s.destPort) ? s.destPort : ''}
                  onChange={(e) => updSub(root, { destPort: numVal(e.target.value) })}
                  className="max-w-[160px]"
                />
              </FieldRow>
            </>
  ...
  if (form.appId === 'iBeaconAdvertisement') {
      ...
      <>
        <FieldRow label="Advertise Interval (ms)" error={errs['ib.interval']} required>
          <Input
            type="number"
            disabled={ro}
            value={Number.isFinite(b.interval) ? b.interval : ''}
            onChange={(e) => updSub('iBeaconAdvertisement', { interval: numVal(e.target.value) })}
            className="max-w-[160px]"
          />
        </FieldRow>
        ...
        <FieldRow label="Major" error={errs['ib.major']} required>
          <Input
            type="number"
            disabled={ro}
            value={Number.isFinite(b.major) ? b.major : ''}
            onChange={(e) => updSub('iBeaconAdvertisement', { major: numVal(e.target.value) })}
            className="max-w-[160px]"
          />
        </FieldRow>
        <FieldRow label="Minor" error={errs['ib.minor']} required>
          <Input
            type="number"
            disabled={ro}
            value={Number.isFinite(b.minor) ? b.minor : ''}
            onChange={(e) => updSub('iBeaconAdvertisement', { minor: numVal(e.target.value) })}
            className="max-w-[160px]"
          />
        </FieldRow>
        <FieldRow label="Measured RSSI (dBm)" error={errs['ib.rssi']} required>
          <Input
            type="number"
            disabled={ro}
            value={Number.isFinite(b.measuredRssi) ? b.measuredRssi : ''}
            onChange={(e) =>
              updSub('iBeaconAdvertisement', { measuredRssi: numVal(e.target.value) })
            }
            className="max-w-[160px]"
          />
        </FieldRow>
      ...
      <>
        <FieldRow label="URL" error={errs['ed.url']} required>
          <Input
            ...
            onChange={(ev) => updSub('eddystoneAdvertisement', { url: ev.target.value })}
            className="max-w-[340px]"
          />
        </FieldRow>
        <FieldRow label="Advertise Interval (ms)" error={errs['ed.interval']} required>
          <Input
            type="number"
            disabled={ro}
            value={Number.isFinite(e.interval) ? e.interval : ''}
            onChange={(ev) =>
              updSub('eddystoneAdvertisement', { interval: numVal(ev.target.value) })
            }
            className="max-w-[160px]"
          />
        </FieldRow>
        <FieldRow label="Measured RSSI (dBm)" error={errs['ed.rssi']} required>
          <Input
            type="number"
            disabled={ro}
            value={Number.isFinite(e.measuredRssi) ? e.measuredRssi : ''}
            onChange={(ev) =>
              updSub('eddystoneAdvertisement', { measuredRssi: numVal(ev.target.value) })
            }
            className="max-w-[160px]"
          />
        </FieldRow>
    ...
    modeBody = scanGroups('iBeaconScan', true, fwdI, setFwdI);
  else if (form.appId === 'eddystoneScan')
    modeBody = scanGroups('eddystoneScan', false, fwdE, setFwdE);
    ...
    const tField = (
      ...
      <FieldRow label={label} error={err} required>
        <Input
          ...
          onChange={(e) =>
            updSub('threadGateway', { [key]: e.target.value } as Partial<ThreadGateway>)
          }
          className={cls}
        />
      </FieldRow>
      ...
      <>
        ...
        <FieldRow label="Channel" error={errs['tg.ch']} required>
          <Input
            type="number"
            disabled={ro}
            value={Number.isFinite(t.channel) ? t.channel : ''}
            onChange={(e) => updSub('threadGateway', { channel: numVal(e.target.value) })}
            className="max-w-[160px]"
          />
        </FieldRow>
