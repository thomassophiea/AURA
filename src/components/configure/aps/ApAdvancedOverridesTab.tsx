    ...
    <div className="space-y-5">
      ...
      {F('AP-LAG') && (
        <OvrRow
          label="LAG"
          overridden={!!d.lagOvr}
          onOverriddenChange={(v) => upd('lagOvr', v)}
          inheritedDisplay="Inherited from profile"
        >
      ...
      <OvrRow
        label="Eth1 Port Function"
        overridden={!!d.ge2modeOvr}
        onOverriddenChange={(v) => upd('ge2modeOvr', v)}
        inheritedDisplay="Inherited from profile"
      >
        <ApSelect
          className="w-40"
      ...
      {F('USB') && (
        <OvrRow
          label="USB Power"
          overridden={!!d.usbPowerOvr}
          onOverriddenChange={(v) => upd('usbPowerOvr', v)}
          inheritedDisplay="Inherited from profile"
        >
          <ApSelect
            className="w-32"
      ...
      {F('PSE') && (
        <OvrRow
          label="PSE Power"
          overridden={!!d.psePowerOvr}
          onOverriddenChange={(v) => upd('psePowerOvr', v)}
          inheritedDisplay="Inherited from profile"
        >
          <ApSelect
            className="w-32"
      ...
      <OvrRow
        label="SSH Access"
        overridden={!!d.sshEnabledOvr}
        onOverriddenChange={(v) => upd('sshEnabledOvr', v)}
        inheritedDisplay="Inherited from profile"
      >
        <ApSelect
          className="w-36"
          value={d.sshEnabled ? 'Enabled' : 'Disabled'}
          options={['Enabled', 'Disabled']}
          onChange={(v) => upd('sshEnabled', v === 'Enabled')}
        />
      </OvrRow>
      ...
      <OvrRow
        label="Event Level"
        overridden={!!d.apLogLevelOvr}
        onOverriddenChange={(v) => upd('apLogLevelOvr', v)}
        inheritedDisplay="Inherited from profile"
      >
        <ApSelect
          className="w-40"
      ...
      {F('EDGE-COMPUTE') && (
        <OvrRow
          label="Edge Compute"
          overridden={!!d.edgeOvr}
          onOverriddenChange={(v) => upd('edgeOvr', v)}
          inheritedDisplay="Inherited from profile"
        >
      ...
      {F('POLL-TIMEOUT') && (
        <OvrRow
          ...
          overridden={!!d.pollTimeoutOvr}
          onOverriddenChange={(v) => upd('pollTimeoutOvr', v)}
          inheritedDisplay="Inherited from profile"
        >
          <NumberField
            value={d.pollTimeout}
      ...
      {F('FABRIC-ATTACH-AUTH-KEY') && (
        <OvrRow
          label="FA Authentication Key"
          overridden={!!d.faAuthKeyOvr}
          onOverriddenChange={(v) => upd('faAuthKeyOvr', v)}
          inheritedDisplay="Inherited from profile"
        >
          <MaskedInput
            className="w-56"
            value={d.faAuthKey ?? ''}
            onChange={(v) => upd('faAuthKey', v)}
          />
        </OvrRow>
      )}
      {F('LED-CONTROL') && (
        <OvrRow
          label="LED Status"
          overridden={!!d.ledStatusOvr}
          onOverriddenChange={(v) => upd('ledStatusOvr', v)}
          inheritedDisplay="Inherited from profile"
        >
          <ApSelect
            className="w-36"
            ...
            onChange={(v) => upd('ledStatus', v)}
          />
        </OvrRow>
      )}
      <OvrRow
        label="Preferred Connection"
        overridden={!!d.affinityOvr}
        onOverriddenChange={(v) => upd('affinityOvr', v)}
        inheritedDisplay="Inherited from profile"
      >
        <ApSelect
          className="w-36"
          ...
          onChange={(v) => upd('affinity', v)}
        />
      </OvrRow>

      <OvrRow
        label="PEAP Username"
        overridden={!!d.peapUsernameOvr}
        onOverriddenChange={(v) => upd('peapUsernameOvr', v)}
        inheritedDisplay="Inherited from profile"
      >
        <div className="flex items-center gap-2">
          <ApSelect
            className="w-40"
            value={gs('peapUsername.selection')}
            options={PEAP_OPTS}
            onChange={(v) => upd('peapUsername.selection', v)}
          />
          {g('peapUsername.selection') === 'Custom' && (
            <Input
              className="w-40"
              value={gs('peapUsername.custom')}
              onChange={(e) => upd('peapUsername.custom', e.target.value)}
            />
          )}
        </div>
      </OvrRow>
      <OvrRow
        label="PEAP Password"
        overridden={!!d.peapPasswordOvr}
        onOverriddenChange={(v) => upd('peapPasswordOvr', v)}
        inheritedDisplay="Inherited from profile"
      >
        <div className="flex items-center gap-2">
          <ApSelect
            className="w-40"
            value={gs('peapPassword.selection')}
            options={PEAP_OPTS}
            onChange={(v) => upd('peapPassword.selection', v)}
          />
          {g('peapPassword.selection') === 'Custom' && (
            <MaskedInput
              className="w-40"
              value={gs('peapPassword.custom')}
              onChange={(v) => upd('peapPassword.custom', v)}
            />
          )}
        </div>
      </OvrRow>
      ...
      {F('PKI-CERT') && (
        <OvrRow
          label="Enforce Manufacturing Certificate"
          overridden={!!d.enforcePkiAuthOvr}
          onOverriddenChange={(v) => upd('enforcePkiAuthOvr', v)}
          inheritedDisplay="Inherited from profile"
        >
      ...
      <OvrRow
        label="Client Bridge RSS Threshold [dBm]"
        overridden={!!d.cbRssThresholdOvr}
        onOverriddenChange={(v) => upd('cbRssThresholdOvr', v)}
        inheritedDisplay="Inherited from profile"
      >
        <NumberField
          value={d.cbRssThreshold}
          min={-100}
          max={-40}
          onChange={(v) => upd('cbRssThreshold', v)}
        />
      </OvrRow>
      ...
      {F('SMART-POLL') && (
        <>
          ...
          <OvrRow
            label="Smart Poll Override"
            overridden={!!d.smartPollOvr}
            onOverriddenChange={(v) => upd('smartPollOvr', v)}
            inheritedDisplay="Inherited from profile"
          >
            <div className="space-y-2">
              <div className="flex items-center gap-3">
                ...
                <Switch
                  checked={!!sp.enabled}
                  onCheckedChange={(v) => upd('smartPoll.enabled', v)}
                />
              </div>
              {sp.enabled && (
                <div className="space-y-2">
                  <div className="flex items-center gap-3">
                    ...
                    <ApSelect
                      ...
                      onChange={(v) => upd('smartPoll.interval', Number(v))}
                    />
                    <span className="text-xs text-muted-foreground">Deadline [s]</span>
                    <NumberField
                      className="w-24"
                      value={sp.deadline}
                      onChange={(v) => upd('smartPoll.deadline', v)}
                    />
                  </div>
                  <Input
                    className="w-full"
      ...
      {(d.radios ?? []).map((r, i) => (
        <div key={i} className="space-y-3 rounded-md border border-border p-3">
          ...
          <OvrRow
            ...
            overridden={gb(`radios.${i}.probeSuppOnLowRssOvr`)}
            onOverriddenChange={(v) => upd(`radios.${i}.probeSuppOnLowRssOvr`, v)}
            inheritedDisplay="Inherited"
          >
            <Switch
              checked={gb(`radios.${i}.probeSuppOnLowRss`)}
              onCheckedChange={(v) => upd(`radios.${i}.probeSuppOnLowRss`, v)}
            />
          </OvrRow>
          {F('FORCE-DEASSOC-ON-LOW-RSS') && (
            <OvrRow
              label="Disassociate on low RSS"
              overridden={gb(`radios.${i}.deasscOnLowRssOvr`)}
              onOverriddenChange={(v) => upd(`radios.${i}.deasscOnLowRssOvr`, v)}
              inheritedDisplay="Inherited"
            >
              <Switch
                checked={gb(`radios.${i}.deasscOnLowRss`)}
                onCheckedChange={(v) => upd(`radios.${i}.deasscOnLowRss`, v)}
              />
            </OvrRow>
          )}
          <OvrRow
            label="RSS Threshold [dBm]"
            overridden={gb(`radios.${i}.probeSuppRssThOvr`)}
            onOverriddenChange={(v) => upd(`radios.${i}.probeSuppRssThOvr`, v)}
            inheritedDisplay="Inherited"
          >
            <NumberField
              value={gn(`radios.${i}.probeSuppRssTh`)}
              min={-100}
              max={-50}
              onChange={(v) => upd(`radios.${i}.probeSuppRssTh`, v)}
            />
          </OvrRow>
          <OvrRow
            label="Max Probe Retries"
            overridden={gb(`radios.${i}.maxProbeRtyOvr`)}
            onOverriddenChange={(v) => upd(`radios.${i}.maxProbeRtyOvr`, v)}
            inheritedDisplay="Inherited"
          >
            <NumberField
              value={gn(`radios.${i}.maxProbeRty`)}
              min={1}
              max={10}
              onChange={(v) => upd(`radios.${i}.maxProbeRty`, v)}
            />
          </OvrRow>
          <OvrRow
            label="RSS Offset [dB]"
            overridden={gb(`radios.${i}.rssOffsetOvr`)}
            onOverriddenChange={(v) => upd(`radios.${i}.rssOffsetOvr`, v)}
            inheritedDisplay="Inherited"
          >
            <NumberField
              value={gn(`radios.${i}.rssOffset`)}
              min={0}
              max={120}
              onChange={(v) => upd(`radios.${i}.rssOffset`, v)}
            />
          </OvrRow>
        </div>
      ))}
