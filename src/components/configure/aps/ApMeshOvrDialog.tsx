...
export function ApMeshOvrDialog({
  mp,
  features,
  open,
  onOpenChange,
  onApply,
}: ApMeshOvrDialogProps) {
      ...
      {F('MESHPOINT-BEACON') &&
        ...
        ov(
          ...
          <Switch
            ...
            onCheckedChange={(v) => upd('monitorPrimaryLink', v)}
          />
        )}
      {ov(
        ...
        <ApSelect
          ...
          options={['None', 'AutoMint', 'AutoProximity']}
          onChange={(v) => upd('rootSelectionMethod', v)}
        />
      )}
      {!F('MESHPOINT-BEACON') &&
        ov(
          ...
          <ApSelect
            ...
            options={['Uniform', 'SNRLeaf', 'MobileSNRLeaf', 'BoundPair']}
            onChange={(v) => upd('pathSelectionMethod', v)}
          />
        )}
      {ov(
        'Preferred Band',
        ...
        <ApSelect
          ...
          options={PREF_BAND}
          onChange={(v) => upd('preferredBand', v)}
        />
      )}
      {ov(
        'Preferred Neighbor',
        ...
        <Input
          ...
          onChange={(e) => upd('preferredNeighbor', e.target.value || null)}
        />
      )}
      {ov(
        'Preferred Root',
        ...
        <Input
          ...
          placeholder="BSSID (aa:bb:cc:dd:ee:ff)"
          onChange={(e) => upd('preferredRoot', e.target.value || null)}
        />
      )}
      {ov(
        'Monitor CRM',
        'monitorCrm',
        <Switch checked={!!d.monitorCrm} onCheckedChange={(v) => upd('monitorCrm', v)} />
      )}
      {ov(
        'Cost Root',
        'costRoot',
        <Switch checked={!!d.costRoot} onCheckedChange={(v) => upd('costRoot', v)} />
      )}
      {ov(
        'Exclude Wired Peer',
        'excludeWiredPeer',
        <Switch
          ...
          onCheckedChange={(v) => upd('excludeWiredPeer', v)}
        />
      )}
      {ov(
        'Hysteresis Min Threshold [dBm]',
        'hysteresisMinTh',
        <NumberField value={d.hysteresisMinTh} onChange={(v) => upd('hysteresisMinTh', v)} />
      )}
      {ov(
        'Hysteresis Period [s]',
        'hysteresisPeriod',
        <NumberField value={d.hysteresisPeriod} onChange={(v) => upd('hysteresisPeriod', v)} />
      )}
      {ov(
        'Hysteresis Delta [dB]',
        'hysteresisDelta',
        <NumberField value={d.hysteresisDelta} onChange={(v) => upd('hysteresisDelta', v)} />
      )}
      {ov(
        'Hysteresis SNR Delta [dB]',
        'hysteresisSNRDelta',
        <NumberField value={d.hysteresisSNRDelta} onChange={(v) => upd('hysteresisSNRDelta', v)} />
      )}
        ...
        {(d.bandSettings ?? []).map((b: ApMeshpointBandSetting, i) => (
          <div key={b.bandId} className="space-y-3 rounded-md border border-border p-3">
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              <BandCell
                label="ACS Channel Plan"
                ovr={!!b.acsPlanOvr}
                onOvr={(v) => upd(`bandSettings.${i}.acsPlanOvr`, v)}
              >
                <ApSelect
                  className="w-44"
                  value={b.acsPlan}
                  options={plansFor(b.bandId)}
                  onChange={(v) => upd(`bandSettings.${i}.acsPlan`, v)}
                />
              ...
              <BandCell
                label="Tx Power"
                ovr={!!b.txPowerOvr}
                onOvr={(v) => upd(`bandSettings.${i}.txPowerOvr`, v)}
              >
                <NumberField
                  value={b.txPower}
                  onChange={(v) => upd(`bandSettings.${i}.txPower`, v)}
                />
              </BandCell>
              <BandCell
                label="Path Min"
                ovr={!!b.pathMinOvr}
                onOvr={(v) => upd(`bandSettings.${i}.pathMinOvr`, v)}
              >
                <NumberField
                  value={b.pathMin}
                  onChange={(v) => upd(`bandSettings.${i}.pathMin`, v)}
                />
              </BandCell>
              <BandCell
                label="Path Threshold"
                ovr={!!b.pathThresholdOvr}
                onOvr={(v) => upd(`bandSettings.${i}.pathThresholdOvr`, v)}
              >
                <NumberField
                  value={b.pathThreshold}
                  onChange={(v) => upd(`bandSettings.${i}.pathThreshold`, v)}
                />
              </BandCell>
              <BandCell
                label="Tolerance [s]"
                ovr={!!b.tolerancePeriodOvr}
                onOvr={(v) => upd(`bandSettings.${i}.tolerancePeriodOvr`, v)}
              >
                <NumberField
                  value={b.tolerancePeriod}
                  onChange={(v) => upd(`bandSettings.${i}.tolerancePeriod`, v)}
                />
              </BandCell>
            </div>
          </div>
