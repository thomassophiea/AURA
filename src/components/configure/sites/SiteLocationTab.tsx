/**
 * Site editor · Location tab (golden SITE_TABS "Location" + the identity
 * Country/Timezone selects, moved here to keep General uncluttered):
 * country, timezone, type-of-place, site manager fields, region/city/campus,
 * postal code, treeNode.mapCoordinates with lat/long validation, AFC scheduled
 * update (afcUpdate.hour/minute) and 802.11mc FTM AP-to-AP ranging (apRanging).
 */
...
export function SiteLocationTab({ form, update, errs }: SiteTabProps) {
      ...
      <Section title="Region & Time">
        ...
        <FieldRow label="Timezone">
          <Select
            ...
            onValueChange={(v) => update('timezone', v === NONE ? '' : v)}
          >
      ...
      <Section title="Place">
        ...
        <FieldRow label="Site Manager Name">
          <Input
            ...
            onChange={(e) => update('siteManagerName', e.target.value)}
        ...
        <FieldRow label="Site Manager Email">
          <Input
            ...
            onChange={(e) => update('siteManagerEmail', e.target.value)}
            className="max-w-[300px]"
          />
        </FieldRow>
        <FieldRow label="Region">
          <Input
            value={tn('region')}
            onChange={(e) => update('treeNode.region', e.target.value)}
        ...
        <FieldRow label="City">
          <Input
            value={tn('city')}
            onChange={(e) => update('treeNode.city', e.target.value)}
            className="max-w-[260px]"
          />
        </FieldRow>
        <FieldRow label="Campus">
          <Input
            value={tn('campus')}
            onChange={(e) => update('treeNode.campus', e.target.value)}
            className="max-w-[260px]"
          />
        </FieldRow>
