...
export function DeviceAdvancedDialog({
  open,
  form,
  F,
  setPath,
  onClose,
}: DeviceAdvancedDialogProps) {
  const sp = (form.smartPoll ?? {}) as Partial<SmartPollConfig>;
  const mtuMax = F('JUMBO-FRAMES') ? 1800 : 1500;
  const mtuErr =
    form.mtu != null && !inRange(form.mtu, 600, mtuMax) ? `Valid range 600 to ${mtuMax}` : null;
  const pollErr =
    F('POLL-TIMEOUT') && !inRange(form.pollTimeout, 3, 600) ? 'Valid range 3 to 600' : null;
  const cbErr =
    form.cbRssThreshold != null && !inRange(form.cbRssThreshold, -128, -40)
      ? 'Valid range -128 to -40'
      : null;
  const spDeadlineErr =
    F('SMART-POLL') && sp.enabled && !inRange(sp.deadline, 1, Number(sp.interval) || 300)
      ...
      : null;
  const spTargetsErr =
    F('SMART-POLL') && sp.enabled && (sp.targets ?? []).length > 10 ? 'Maximum 10 targets' : null;
  const selDnsErr = selDnsError(form.selDnsIntercept) ? SEL_DNS_HINT : null;
  const peapUErr =
    getIn(form, 'peapUsername.selection') === 'Custom' && !getIn(form, 'peapUsername.custom')
      ? 'Custom username is required'
      : null;
  const peapPErr =
    getIn(form, 'peapPassword.selection') === 'Custom' && !getIn(form, 'peapPassword.custom')
      ? 'Custom password is required'
      : null;

  const num = (key: string) => (
    <NumInput
      value={getIn(form, key) as number}
  ...
  const sel = (key: string, opts: Opt[], w = 'w-52') => (
    <PSelect
      value={(getIn(form, key) as string) ?? ''}
      options={opts}
      onChange={(v) => setPath(key, v)}
      className={w}
      ariaLabel={key}
  ...
  const onoff = (key: string) => (
    <BoolSelect value={!!getIn(form, key)} onChange={(v) => setPath(key, v)} className="w-36" />
  );
  const chk = (key: string) => (
    <Switch
      checked={!!getIn(form, key)}
      onCheckedChange={(v) => setPath(key, v)}
      aria-label={key}
    />
  );
  ...
  const secureOn =
    F('SECURE-TUNNEL') && !!form.secureTunnelMode && form.secureTunnelMode !== 'disabled';
  const sensorCustom =
    F('SENSOR-CUSTOM-SCAN') && !!form.sensorMode && form.sensorMode !== 'Default';
    ...
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-h-[85vh] max-w-2xl overflow-y-auto">
        ...
        <div className="space-y-3">
          <LabelRow label="Client Balancing" labelWidth={LW}>
            {onoff('clientBalancing')}
          </LabelRow>
          {F('SECURE-TUNNEL') && (
            <LabelRow label="Secure Tunnel Mode" labelWidth={LW}>
              {sel('secureTunnelMode', SECURE_TUNNEL_OPTS)}
            </LabelRow>
          )}
          {secureOn && (
            <LabelRow label="Secure Tunnel Lifetime [h]" labelWidth={LW}>
              {num('secureTunnelLifetime')}
            </LabelRow>
          )}
          {F('PKI-CERT') && (
            <LabelRow label="Enforce PKI Authentication" labelWidth={LW}>
              {chk('enforcePkiAuth')}
            </LabelRow>
          )}
          <LabelRow label="SSH Access" labelWidth={LW}>
            {onoff('sshEnabled')}
          </LabelRow>
          {F('EDGE-COMPUTE') && (
            <LabelRow label="Edge Compute" labelWidth={LW}>
              {chk('edge')}
            </LabelRow>
          )}
          {F('SESSION-PERSISTENCE') && (
            <LabelRow label="Session Persistence" labelWidth={LW}>
              {onoff('sessionPersistence')}
            </LabelRow>
          )}
          <LabelRow label="Management VLAN ID" labelWidth={LW}>
            {num('mgmtVlanId')}
          </LabelRow>
          <LabelRow label="Management VLAN Tagged" labelWidth={LW}>
            {chk('mgmtVlanTagged')}
          </LabelRow>
          <LabelRow label="MTU" labelWidth={LW} error={mtuErr}>
            {num('mtu')}
          </LabelRow>
          ...
          {F('AP-LAG') && (
            <LabelRow label="LAG" labelWidth={LW}>
              {chk('lag')}
            </LabelRow>
          )}
          {!F('AP-SINGLE-INF-WIRED-CLIENT') && (
            <LabelRow label="GE2 Port Mode" labelWidth={LW}>
              {sel('ge2mode', ge2opts, 'w-40')}
            </LabelRow>
          )}
          {F('USB') && (
            <LabelRow label="USB Power" labelWidth={LW}>
              {sel(
                'usbPower',
                [
                  { id: 'Off', label: 'Off' },
          ...
          {F('PSE') && (
            <LabelRow label="PSE Power" labelWidth={LW}>
              {sel(
                'psePower',
                [
                  { id: 'Auto', label: 'Auto' },
          ...
          <LabelRow label="AP Log Level" labelWidth={LW}>
            {sel('apLogLevel', AP_LOG_LEVEL_OPTS, 'w-40')}
          </LabelRow>
          {F('POLL-TIMEOUT') && (
            <LabelRow label="Poll Timeout [s]" labelWidth={LW} error={pollErr}>
              {num('pollTimeout')}
            </LabelRow>
          )}
          ...
          {F('LED-CONTROL') && (
            <LabelRow label="LED Status" labelWidth={LW}>
              {sel('ledStatus', LED_STATUS_OPTS, 'w-32')}
            </LabelRow>
          )}
          ...
          <LabelRow label="PEAP Password" labelWidth={LW} error={peapPErr}>
            <div className="flex items-center gap-2">
              ...
              {getIn(form, 'peapPassword.selection') === 'Custom' && (
                <MaskedInput
                  value={(getIn(form, 'peapPassword.custom') as string) || ''}
                  onChange={(v) => setPath('peapPassword.custom', v)}
                />
              )}
            </div>
          </LabelRow>
          ...
          <LabelRow label="RSS Threshold [dBm]" labelWidth={LW} error={cbErr}>
            {num('cbRssThreshold')}
          </LabelRow>
          ...
          {F('SMART-POLL') && (
            <>
              ...
              <LabelRow label="Smart Poll" labelWidth={LW}>
                {chk('smartPoll.enabled')}
              </LabelRow>
              {sp.enabled && (
                <>
                  ...
                  <LabelRow label="Deadline [s]" labelWidth={LW} error={spDeadlineErr}>
                    {num('smartPoll.deadline')}
                  </LabelRow>
