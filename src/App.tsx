...
const SiteGroupSettingsPage = lazy(() =>
  import('./components/SiteGroupSettingsPage').then((m) => ({ default: m.SiteGroupSettingsPage }))
);
...
const SystemBackupManager = lazy(() =>
  import('./components/SystemBackupManager').then((m) => ({ default: m.SystemBackupManager }))
);
const LicenseDashboard = lazy(() =>
  import('./components/LicenseDashboard').then((m) => ({ default: m.LicenseDashboard }))
);
const APFirmwareManager = lazy(() =>
  import('./components/APFirmwareManager').then((m) => ({ default: m.APFirmwareManager }))
);
const NetworkDiagnostics = lazy(() =>
  import('./components/NetworkDiagnostics').then((m) => ({ default: m.NetworkDiagnostics }))
);
const EventAlarmDashboard = lazy(() =>
  import('./components/EventAlarmDashboard').then((m) => ({ default: m.EventAlarmDashboard }))
);
...
const pageInfo = {
  ...
  'report-widgets': {
    ...
    description: 'Real-time analytics and monitoring widgets',
  },
  'pci-report': {
    title: 'PCI DSS Report',
    description: 'Generate PCI DSS compliance reports for cardholder data environments',
  },
  'system-backup': {
    title: 'Backup & Storage',
    description: 'Manage configuration backups and flash memory storage',
  },
  'license-dashboard': {
  ...
  'firmware-manager': {
    ...
    description: 'Manage AP firmware upgrades and schedules',
  },
  'network-diagnostics': {
    title: 'Network Diagnostics',
    description: 'Test network connectivity with ping, traceroute, and DNS',
  },
  ...
  'configure-profiles': {
    ...
    description: 'Configure AP device profiles and assignment hierarchy',
  },
  'configure-rrm': {
    title: 'RF Management',
...
export default function App() {
  ...
  const isPageAllowedByPersona = useCallback(
  ...
  const cortexBaseContext = useMemo(
    (): Partial<CortexPageContext> => ({
      ...
      timeRange: {
        ...
        end: filters.dateTo?.toISOString() ?? new Date().toISOString(),
      },
      filters: { site: filters.site, timeRange: filters.timeRange },
    }),
    [currentPage, filters, siteName, activePersona]
  );
  ...
  useEffect(() => {
      ...
      if (apiService.isAuthenticated()) {
        ...
        try {
          ...
          if (sites && sites.length > 0) {
            ...
            const displayName =
              firstSite.displayName || firstSite.name || firstSite.siteName || 'Site';
    ...
    console.warn = (...args) => {
      ...
      // Never suppress session expired errors for critical endpoints
      if (
        warnString.includes('Session expired') &&
      ...
      case 'global-templates':
        return (
          <GlobalElementsPage
            ...
            initialElementType={pendingTemplateType ?? undefined}
            onConsumeElementType={() => setPendingTemplateType(null)}
          />
        );
