...
describe('CortexEvidenceAccordion', () => {
  it('renders nothing when both lists are empty', () => {
    const { container } = render(<CortexEvidenceAccordion apiEvidenceUsed={[]} missingData={[]} />);
  ...
  it('expands to reveal API paths on click', () => {
    render(<CortexEvidenceAccordion apiEvidenceUsed={['/v1/stations/{mac}']} missingData={[]} />);
    fireEvent.click(screen.getByRole('button'));
    expect(screen.getByText('/v1/stations/{mac}')).toBeInTheDocument();
  });

  it('shows missing data section after expand', () => {
    render(<CortexEvidenceAccordion apiEvidenceUsed={[]} missingData={['/v1/stations/{mac}']} />);
    fireEvent.click(screen.getByRole('button'));
    expect(screen.getByText('/v1/stations/{mac}')).toBeInTheDocument();
  });
});
